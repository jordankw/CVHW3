...
int main(int argc, char** argv) {
	cap = cv::VideoCapture("HW3/input/part5a/input5a2.avi");
	cv::Size csize(cap.get(CV_CAP_PROP_FRAME_WIDTH), cap.get(CV_CAP_PROP_FRAME_HEIGHT));
	cap_out.open((const cv::String)"HW3/output/part5a/output5a2.avi", cap.get(CV_CAP_PROP_FOURCC), cap.get(CV_CAP_PROP_FPS), csize, true);
	bool eof = false;
	float blurSize = 0.1f;
